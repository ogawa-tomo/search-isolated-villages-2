...
def main():
    ...
    for pref in tqdm(RegionSetting.get_all_prefs()):
        ...
        # 施設データの読み込み
        faculties_inf_pref = extract_faculties_by_pref(all_faculties, pref)
...
def extract_faculties_by_pref(all_faculties, pref):

    faculties_in_pref = []
    for f in all_faculties:
        if f.pref == pref:
            faculties_in_pref.append(f)
    return faculties_in_pref
